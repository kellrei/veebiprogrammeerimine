...
//echo $serverHost;
 ...
 function userdescription(){
	 $userprofile = array();
		...
		$stmt = $mysqli->prepare("SELECT description, textcolor, bgcolor FROM VPuserprofiles");
		echo $mysqli -> error;
		
		$stmt->bind_result($descriptionFromDb, $firstnameFromDb, $lastnameFromDb);
 }
  function getuserprofile($userId){
		$userprofile = array();
		$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
		
		$stmt = $mysqli->prepare("SELECT description, textcolor, bgcolor FROM VPuserprofiles WHERE userid = ?");
		echo $mysqli -> error;
		
		$stmt->bind_param("i", $userId);
		$stmt->bind_result($descriptionFromDb, $textcolorFromDb, $bgcolorFromDb);
		$stmt->execute();
		$stmt->fetch();
		
		// Set values to array
		array_push($userprofile, $descriptionFromDb, $textcolorFromDb, $bgcolorFromDb);
  ...
  function setuserprofile($userId, $description, $textcolor, $bgcolor){
		$notice = "";
		$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
		
		$stmt = $mysqli->prepare("REPLACE INTO VPuserprofiles (userid, description, textcolor, bgcolor) VALUES (?, ?, ?, ?)"); 
		echo $mysqli -> error;
		
		$stmt->bind_param("isss", $userId, $description, $textcolor, $bgcolor);
		$stmt->execute();
